!function () {
    // 服务器请求地址
    var host = 'wx.ninebranch.com';
    var href = 'http://' + host + '/';
    var wxappid = 'wx64d06dc5579589ae';
 ...
 */
            ...
            */
             ...
             * 查询最新一条未结束订单
             */
            get_query_using: function (callback) {
            	var token	= $.cookie("token"); 
			    var uid		= $.cookie("uid");			 
				$.ajax({
					url:href+"api/deviceController/queryNewOrderno",
					type:"GET",
					dataType:'json',
					beforeSend: function(request) {
						request.setRequestHeader("token", token);
             ...
             * @brief 获取地图网点列表        
             **/
            get_area_list: function (callback){
            	$.ajax({
            		type: "get",
            		url: href +"api/deviceController/getPartition",
            		async:false,
            		dataType: 'json',
                    success: callback,
                    error: function (err) {
             ...
             * @brief 获取地图网点列表        
             **/
            check_area_detail: function (areaId,callback){
            	$.ajax({
            		url: href +'api/deviceController/getPartitionData',
            		type: 'GET',
            		async: false,
            		data: {
            			id:areaId
            ...
            get_shop_detail: function (shopid, callback) {
                $.ajax({
                    async: false,
                    url: href + "api/deviceController/getHomeDetail",
                    type: 'GET',
                    dataType: 'json',
                    data: {
                        "id": shopid
             ...
             * @brief 设备报损 
             **/
             device_wrong: function (orderno,comment,img, callback) {
             	var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                    async: false,
                    url: href + "api/deviceController/errorDevice",
                    type: 'POST',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
                        request.setRequestHeader("uid", uid);
             ...
             * @brief 订单状态查询
             * */
            return_devices: function (sno,callback) {
            	var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                    url: href + "api/deviceController/returnDevice",
                    type: 'POST',
                    dataType: 'json',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
             ...
             * @brief 订单状态查询
             * */
            check_orderno_status: function (orderNo, callback) {
            	var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                    url: href + "api/deviceController/queryOrderno",
                    type: 'GET',
                    dataType: 'json',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
             ...
             * @brief 收费规则
             * */
            getBorrowRule: function (sno, callback) {
            	var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                    url: href + "api/deviceController/getBorrowRule",
                    type: 'GET',
                    dataType: 'json',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
             ...
             * @brief 获取我的借还记录详情
             **/
            get_my_borrow_details: function (orderno, callback) {
                var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                    url: href + "api/personController/queryBorrowOne",
                    type: 'GET',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
                        request.setRequestHeader("uid", uid);
             ...
             * @brief 充值
             **/
            chargeMoney: function (amount, pay_type, used_to,callback) {
                var token  = $.cookie("token");
                var uid    = $.cookie("uid");
             ...
             * @brief 设备报损
             **/
            errorDevice: function (orderno, callback) {
                $.ajax({
                    url: href + "deviceController/errorDevice",
                    type: 'GET',
                    data: {
                        orderno: orderno
                    },
             ...
             * @brief 设备报失
             **/
            lostDevice: function (orderno, callback) {
            	var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                    url: href + "api/deviceController/lostDevice",
                    type: 'POST',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
                        request.setRequestHeader("uid", uid);
 ...
 */
             ...
             * @brief 用户注册
             */
            userRegister: function (phone, password, code,personPhone,urgentPhone,healthyState, callback) {
                var token = $.cookie("token");
                var uid = $.cookie("uid");
                $.ajax({
                ...
                $.ajax({
                    ...
                    success: function (res) { 
                        ...
                        if (res.status === 0) {
                      	...
                      	localStorage.setItem('openid',res.data.wx)
                        if (callback) {
                            callback(res);
                        }
                    },
             ...
             * @brief 用户主动凭账号登录
             */
            userLogin: function (openid, open_type, callback) {
                var userInfo = {};
                // var token = $.cookie("token");
                // var uid = $.cookie("uid");
                $.ajax({
             ...
             * @brief 获取求助电话
            **/
            getNearPhone: function (sitelat,sitelng,callback) {     
                $.ajax({
                    url: href + "api/systemController/getNearPhone",
                    type: 'GET', 
                   	data: {
                        lat: sitelat,
                        lng: sitelng
 ...
 */
            ...
            /**
             * @brief 唤起微信扫一扫
             * @param {int} type: 0:借 1:还 2报损
             **/
            wakeup_scan_weixin: function (url, type) {
            	console.log(url)
                $.ajax({
                    url: href + "/api/weixinController/getWeChatConfig?url=" + url,
                    type: "GET",
                            ...
                            wx.ready(function () {                      
                                wx.scanQRCode({
                                    ...
                                    success: function (data) {                       	
                                    	mcfish.Handle.handle_scan_result(data.resultStr,type);	// 扫描结果处理函数                                                       
                                    }
                                });
                            });
             ...
             * @param {int} type: 0:借 1:还 2报损
             **/
            getLocation_weixin: function (url, callback) {
            	var sitelat,sitelng
                $.ajax({
                    url: href + "/api/weixinController/getWeChatConfig?url=" + url,
                    type: "GET",
                        ...
                        if (msg.status === 0) {
                            ...
                            wx.ready(function () {
                               wx.getLocation({
									...
									localStorage.setItem('sitelat',latitude)
									localStorage.setItem('sitelng',longitude)
									}
								});
                            });
                            wx.error(function (res) {
             ...
             * @brief 微信地图
             **/
            openlocation: function (url, lat,lng,adrname) {
            	console.log(lat)
            	console.log(lng)
            	console.log(adrname)
            	console.log(url)
                ...
                function onBridgeReady() {
                    WeixinJSBridge.invoke(
                        'getBrandWCPayRequest', {
                        	"debug":true,
                            "appId": data.data.pay.appId, //公众号名称，由商户传入     
                            "timeStamp": data.data.pay.timeStamp, //时间戳，自1970年以来的秒数     
                            "nonceStr": data.data.pay.nonceStr, //随机串     
                            "package": data.data.pay.packAge,
                            "signType": data.data.pay.signType, //微信签名方式：
                            "paySign": data.data.pay.paySign //微信签名 
                        },
                        function (res) {
 ...
 */
            ...
            feedback: function(content, callback){
                ...
                var uid = $.cookie("uid"); 
                $.ajax({
                    url: href + "api/systemController/feedback",
                    type: 'POST',
                    beforeSend: function (request) {
                        request.setRequestHeader("token", token);
                        request.setRequestHeader("uid", uid);
 ...
 */
        ...
        Handle: {
             ...
             * @brief 处理扫描结果
             * */
            handle_scan_result: function (url, type) {
                // 解析扫描的结果
               ...
               // 借的逻辑
				if (type == 0){
					// 成功获取到设备的序列码，跳转到选择产品页面
					var sno = request.params['sno'];
					window.location.href = 'product_type.html?sno='+sno;
				}
				// 还的逻辑
				if (type == 1){
					// 成功获取到设备的序列码，执行还设备的逻辑
					var sno = request.params['sno'];
					window.location.href = 'return_result.html?sno='+sno;
				}
				//报损
				if (type == 2){
					var sno = request.params['sno'];
            ...
            handle_device_status: function (data, sno) {
                switch (data.status) {
                    ...
                    case 506: // 设备离线
                        alert('您有未结算的订单!');
                        window.location.href = destHost + 'app/pages/user/useageLog.html';
                        break;
                    case 507: // 设备不存在
                        alert('此设备不存在！');
                        break;
